...
class Arguments(TrainingArguments):
    model_name_or_path: str = field(
        default='intfloat/e5-base-unsupervised',
        metadata={"help": "Path to pretrained model or model identifier from huggingface.co/models"}
    )

    data_dir: str = field(
        default='Data/Raw Datasets', metadata={"help": "Path to train directory"}
    )
    task_type: str = field(
        default='ir', metadata={"help": "task type: ir / qa"}
    )
    train_file: Optional[str] = field(
        default=','.join(['Data/Datasets Info/raw_dataset_'+dataset_name+'.jsonl' for dataset_name in ['msmarco', 'nli', 'nq']]), metadata={"help": "The input training data file (a jsonlines file)."}
    )
    ...
    do_kd_biencoder: bool = field(default=True, metadata={"help": "knowledge distillation to biencoder"})
